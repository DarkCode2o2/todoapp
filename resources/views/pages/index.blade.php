...
<div class="tasks">
    ...
    @if (count($tasks) > 0)
        <div class="flex justify-between items-start flex-col lg:flex-row gap-3">
            <div class="w-full">
                <p class="font-semibold text-blue-500 mb-4">
                ...
                @foreach ($tasks as $task)
                    <a href="/tasks/{{$task->id}}" class="task w-full relative inline-block border-1 border-gray-300 rounded p-2 mb-4 pl-7 hover:bg-blue-100 transition-all">
                        @if($task->is_done)
            ...
            <div class="w-full">
                ...
                <div class="border-1 border-gray-300 w-full flex justify-around min-h-[200px] p-4 ">
                    <div>
                        <div class="relative flex items-center justify-center w-30 h-30 sm:w-40 sm:h-40 rounded-full 
                                    [background:conic-gradient(#3b82f6_calc(var(--progress)_*_1%),#e5e7eb_0)]"
                                    style="--progress: {{$unDoneTasks}}">
                            <span class="absolute flex items-center justify-center w-20 h-20 sm:w-30 sm:h-30 rounded-full bg-white text-xl font-bold">
                            {{$unDoneTasks}}%
                        </span>
                        </div>
                        <p class="font-semibold text-sm text-blue-400 mt-2 text-center">Uncompeleted Tasks</p>
                    </div>
                    <div>
                        <div class="relative flex items-center justify-center w-30 h-30 sm:w-40 sm:h-40  rounded-full 
                                    [background:conic-gradient(tomato_calc(var(--progress)_*_1%),#e5e7eb_0)]"
                                    style="--progress: {{$doneTasks}}">
                            <span class="absolute flex items-center justify-center w-20 h-20 sm:w-30 sm:h-30 rounded-full bg-white text-xl font-bold">
                            {{$doneTasks}}%
                        </span>
                        </div>
                        <p class="font-semibold text-sm text-red-500 mt-2 text-center">Compeleted Tasks</p>
                    </div>
                </div>
            </div>
        </div>
